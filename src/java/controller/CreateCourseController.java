...
public class CreateCourseController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (acc == null) {
            ...
            Boolean visibleto = request.getParameter("visibleto")!=null&&request.getParameter("visibleto").equals("0")?false:true;
            int courseid = cd.insertCourse(new Course(title, des, visibleto, acc.getAccountid()));
            int i = 1;
            while(request.getParameter("term"+i)!=null&&!request.getParameter("term"+i).equals("")){
                String term = request.getParameter("term"+i);
                String def = request.getParameter("def"+i);
                Question q = new Question( null, term, def, courseid);
                qd.insertQuestion(q);
                i++;
            }
